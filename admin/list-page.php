...
function wikiembed_list_page() {
	...
	<div class="wrap">
		...
		<form method="post" acction=""> 
			<ul class="subsubsub">
				<li>
					<a href="?page=wiki-embed" <?php echo ( ! isset( $_GET['non_url_items'] ) ? 'class="current"' : '' ); ?>>
						All <span class="count">(<?php echo $total_parent_count; ?>)</span>
					</a> |
				</li>
				<li>
					<a href="?page=wiki-embed&non_url_items=true" <?php echo ( ! isset( $_GET['non_url_items'] ) ? 'class="current"' : '' ); ?>>
						No Target URL <span class="count">(<?php echo $count_non_url_items;?>)</span>
					</a>
				</li>
			</ul>
			<div class="tablenav">
					...
					<?php
						if ( $wikiembeds_parents ):
								...
								<tr valign="top" class="<?php echo ( ($i % 2) ? 'alternate': ''); ?> parent" >
									...
									<td>
										...
										<?php else: //REMOVE ?>
											<p>
												<span class="spacer">
													<a href="<?php echo esc_url( $item['url'] ); ?>"><?php echo $item['url']; ?></a>
												</span> 
							...
							<tr valign="top" class="alternate">
							   <td></td>
							   <td>You don't have any Wiki Embeds Stored <br />Try embeding a wiki using a shortcode. </td>
							   <td></td>
							   <td></td>
						   </tr>
						   <?php 
