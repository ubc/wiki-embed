 ...
 * Plugin URI: https://github.com/ubc/wiki-embed
...
function wikiembed_init() {
	...
	if ( ! is_admin() ): // never display this stuff in the admin 
		...
		case "overlay":
			...
			wp_localize_script( 'wiki-embed-overlay', 'WikiEmbedSettings', array( 'wiki_embed_ajaxurl' => admin_url('admin-ajax.php') ) );
			break;
		case "new-page":
			...
			wp_localize_script( 'wiki-embed-new-page', 'WikiEmbedSettings', array( 'siteurl' => get_site_url() ) );
			break;
		default:
		endswitch;
...
function wikiembed_load_page() {
	...
	switch ( $wikiembed_options['default']['tabs'] ):
	case 1:
		$tabs = true;
		break;
	case 2:
	...
	if ( is_email( $wikiembed_options['wiki-links-new-page-email'] )
		&& ! isset( $_COOKIE["wiki_embed_urls_emailed:".$flat_url] )
