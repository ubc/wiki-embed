...
Class Wiki_Embed{
	...
	function init() {
		...
		// add_action('save_post',array( $this, 'save_post') ); // we don't need this any more
	...
	function default_settings(){
		...
		$wikiembed_options['default']['tabs'] = 1; // 1 = tabs, 2 = accordions
	...
	function get_page_shortcode( $url, $has_accordion, $has_tabs, $has_no_contents, $has_no_edit, $has_no_infobox ) {
		...
		$wiki_embed_shortcode =  "[wiki-embed url=".$url;
...
function make_safe( $body ) {
	...
	function overlay_ajax() {
		...
		if($this->options['default']['tabs']==2)
			$wiki_page_id .= "accordion,";
			
		if($this->options['default']['tabs']==1)
			$wiki_page_id .= "tabs,";
		
		if($this->options['default']['no-contents'])
			$wiki_page_id .= "no-contents,";
			
		if($this->options['default']['no-infobox'])
			$wiki_page_id .= "no-infobox,";
		
		if($this->options['default']['no-edit'])
			$wiki_page_id .= "no-edit,";
	...
	/**
	 * Caching Function for the wiki embed are abstracted 
	 *
	 **/
