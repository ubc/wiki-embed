	...
	$('a.remove-link').live( "click", function() {
		...
		jQuery.post( ajaxurl, data, function(response) {
			if ( response == "success" ) {
				...
				// change the edit to 
				grandparent.children('a').html("Add Target URL");
				// change the Button 
				grandparent.next().children('input.button').val("Add Target URL");
				// replace the the link with 
				grandparent.children(".spacer").html("none");
				grandparent.children(".divider").remove();
